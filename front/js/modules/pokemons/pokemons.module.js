...
import { deletePokemonInTeam, getAllPokemons, getOnePokemon, getOnePokemonByName, voteForPokemon } from "./pokemons.api.js";
...
async function displayPokemonStats(id) {
    ...
    try {
        ...
        document.querySelector('.nmb_of_votes').textContent = `Nombre de votes : ${pokemon.nmbr_of_votes}`
...
async function openPokemonDetailModal(event = null, pokemonId = null) {
    ...
    if(event && event.target) {
        pokemonDetailModal.dataset.id = event.target.closest('.card').dataset.pokemonId;
