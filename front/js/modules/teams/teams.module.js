...
import { handleDeletePokemonInTeam, openPokemonDetailModal } from "../pokemons/pokemons.module.js";
import { getOneTeam, getAllTeams, createTeam, putPokemonInTeam, deleteTeam, editTeam } from "./teams.api.js";
...
function addTeamsToDom(team) {
    ...
    for (const pokemon of team.pokemons) {
        ...
        figure.classList.add('has-text-centered', 'pt-4', 'is-256x256', 'mb-6', 'card', 'pokemon-team-card', 'pokemon-card', 'is-col-min-1');
...
async function openTeamModal(event) {
    ...
    document.getElementById('edit_team_modal').dataset.teamId = event.target.closest('[data-team-id]').dataset.teamId;
...
async function handleCreateTeam(event) {
    try {
        ...
        let message = document.getElementById('nameErrorMessage');
...
async function insertPokemonInTeam(event) {
    ...
    const isVerificationOk = await verificationsBeforeInsertPokemonInTeam(form, teamId, pokemonId);
...
async function verificationsBeforeInsertPokemonInTeam(form, teamId, pokemonId) {
    try {
        ...
        const isPokemonInTeam = team.pokemons.find(pokemon => pokemon.id === parseInt(pokemonId, 10));

        if (isPokemonInTeam) {
            isVerificationOk = false;
            message.textContent = 'Le pokémon est déjà dans l\'équipe, veuillez en choisir un autre.';
            return;
        }

        if (team.pokemons.length >= 6) {
            isVerificationOk = false;
            message.textContent = 'L\'équipe est déjà au complet, veuillez entrer ce pokémon dans une autre équipe.';
            return;
        }
...
export { addTeamsToDom, fetchAndDisplayTeams, openAddTeamModal, handleCreateTeam, insertPokemonInTeam, handleDeleteTeam, openEditTeamModal, handleEditTeam };