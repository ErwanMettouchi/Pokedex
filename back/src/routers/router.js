...
router.get(
	...
	controllerWrapper(pokemonController.getAllPokemonsOrderedByVotes),
);
...
router.post(
	"/pokemons/:id(\\d+)/votes",
	controllerWrapper(pokemonController.voteForPokemon),
);

router.route("/types/").get(controllerWrapper(typeController.getAll));
...
router
	.route("/teams/:teamId(\\d+)/pokemons/:pokemonId(\\d+)")
